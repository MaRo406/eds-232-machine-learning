...
```{r}
xgb_model = boost_tree(learn_rate = tune(), trees = tune()) %>% #set boosted trees model to tune number of trees and number of predictors sampled
...
After creating my workflow for my boosted tree model, I will be using `tune_grid()` to get 10 possible values for both mtry and trees parameters and using cross validation to fit the model to the folds in order to determine the best value for the parameters.
...
xgb_final = finalize_workflow(xgb_workflow, select_best(xgb_cv_tune, metric = "roc_auc")) #finalize random forest model with best parameters
```
